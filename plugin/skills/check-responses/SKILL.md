...
In parallel, call:
- `get_document` with `projectId: "$0"` and `slug: "sales_strategy"`
- `get_master_document` with `slug: "ref_scheduling_services"` (canonical service-name → notification-domain mapping)
...
Call `get_recent_outreach` with `projectId: "$0"`.
...
For each response found, call `record_response` with:
...
**Edge case override**: If the automatic status is incorrect (e.g., a negative-sentiment reply that's actually a "not now, try again later"), use `update_prospect_status` to set the correct status.
