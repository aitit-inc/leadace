...
describe('assessVitals', () => {
  it('reports insufficient below the minimum send floor, even at zero replies', () => {
    const v = assessVitals({ sends: 99, replies: 0 }, DEFAULTS, seededRng('t'))
  ...
  it('reports insufficient at zero sends with a flat posterior', () => {
    const v = assessVitals({ sends: 0, replies: 0 }, DEFAULTS, seededRng('t'))
    expect(v.verdict).toBe('insufficient')
  ...
  it('stays ok at the send floor with zero replies (posterior mass still above the line)', () => {
    ...
    const v = assessVitals({ sends: 100, replies: 0 }, DEFAULTS, seededRng('t'))
  ...
  it('turns futile once zero replies persist well past the floor', () => {
    ...
    const v = assessVitals({ sends: 400, replies: 0 }, DEFAULTS, seededRng('t'))
  ...
  it('a single reply pulls the same volume back from futile', () => {
    ...
    const v = assessVitals({ sends: 400, replies: 1 }, DEFAULTS, seededRng('t'))
    expect(v.verdict).toBe('ok')
  })

  it('is deterministic for a given seed', () => {
    const a = assessVitals({ sends: 400, replies: 0 }, DEFAULTS, seededRng('vitals:proj'))
    const b = assessVitals({ sends: 400, replies: 0 }, DEFAULTS, seededRng('vitals:proj'))
    expect(a).toEqual(b)
  })
  ...
  it('sits within Monte Carlo noise of the confidence gate at 298 zero-reply sends', () => {
    ...
    const v = assessVitals({ sends: 298, replies: 0 }, DEFAULTS, seededRng('a'))
  ...
  it('clamps replies above sends instead of feeding Beta a non-positive shape', () => {
    const v = assessVitals({ sends: 100, replies: 150 }, DEFAULTS, seededRng('t'))
