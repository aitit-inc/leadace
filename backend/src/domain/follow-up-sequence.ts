...
// gapDays are RELATIVE waits (days) before each next touch, not absolute from
// first send, so an irregular /daily-cycle cadence preserves spacing. Default
// [3] = touches at day 0/3; maxTouches = gapDays.length + 1.
...
export const followUpSequenceSchema = z.object({
  ...
  gapDays: gapDaysSchema.default([3]),
})
