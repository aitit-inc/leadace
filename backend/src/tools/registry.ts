// Tool surface shared by the MCP worker (src/mcp/index.ts, src/mcp/stdio.ts)
// and the hosted chat agent (services/chat). Tools never touch services or the
...
export function buildToolRegistry(): ToolDef[] {
  ...
  defineTool(
    ...
    async (input, ctx) => {
      const { ok, data } = await ctx.callApi('POST', '/bug-reports', input)
      if (!ok) {
  ...
  defineTool(
    ...
    {},
    async (_args, ctx) => {
      const { ok, data } = await ctx.callApi('GET', '/projects', null)
      if (!ok) {
  ...
  defineTool(
    ...
    async ({ name }, ctx) => {
      const { ok, data } = await ctx.callApi('POST', '/projects', { name })
      if (!ok) {
        const err = data as { error: string; detail?: string }
  ...
  defineTool(
    ...
    async ({ projectId }, ctx) => {
      const { ok, data } = await ctx.callApi('DELETE', `/projects/${encodeURIComponent(projectId)}`, null)
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    async ({ projectId, prospects }, ctx) => {
      const { ok, data } = await ctx.callApi('POST', '/prospects/batch', { projectId, prospects })
      if (!ok) {
  ...
  defineTool(
    ...
    async ({ projectId, candidates }, ctx) => {
      const { ok, data } = await ctx.callApi('POST', '/prospects/check-dedup', { projectId, candidates })
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    async ({ projectId, csvText, dedupPolicy }, ctx) => {
      const { ok, data } = await ctx.callApi(
        'POST',
        '/prospects/import',
  ...
  defineTool(
    ...
    {},
    async (_args, ctx) => {
      const { ok, data } = await ctx.callApi('GET', '/country-codes', null)
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    async ({ projectId, limit }, ctx) => {
      const { ok, data } = await ctx.callApi('GET', `/projects/${encodeURIComponent(projectId)}/prospects/reachable?limit=${limit}`, null)
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    async ({ projectId }, ctx) => {
      const { ok, data } = await ctx.callApi('GET', `/projects/${encodeURIComponent(projectId)}/mailbox-health`, null)
      if (!ok) {
        const err = data as { error: string; detail?: string }
  ...
  defineTool(
    ...
    async (input, ctx) => {
      const { ok, data } = await ctx.callApi('POST', '/outreach/record-with-inquiry', input)
      if (!ok) {
        const err = data as { error: string; detail?: string }
        const msg = err.detail ? `${err.error}: ${err.detail}` : err.error
        return { content: [{ type: 'text' as const, text: `Error: ${msg}` }], isError: true }
  ...
  defineTool(
    ...
    async ({ outreachLogId, status, errorMessage }, ctx) => {
      ...
      const { ok, data } = await ctx.callApi('PATCH', `/outreach/${outreachLogId}/status`, body)
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    {},
    async (_args, ctx) => {
      const { ok, data } = await ctx.callApi('GET', '/tenant/compliance-status', null)
      if (!ok) {
        const e = data as { error: string; detail?: string }
        const msg = e.detail ? `${e.error}: ${e.detail}` : e.error
        return { content: [{ type: 'text' as const, text: `Error: ${msg}` }], isError: true }
  ...
  defineTool(
    ...
    async ({ projectId }, ctx) => {
      const { ok, data } = await ctx.callApi('GET', `/projects/${encodeURIComponent(projectId)}/message-variants`, null)
      if (!ok) {
        const e = data as { error: string; detail?: string }
        const msg = e.detail ? `${e.error}: ${e.detail}` : e.error
        return { content: [{ type: 'text' as const, text: `Error: ${msg}` }], isError: true }
  ...
  defineTool(
    ...
    async (input, ctx) => {
      ...
      const { ok, data } = await ctx.callApi('POST', path, null)
      if (!ok) {
        const e = data as { error: string; detail?: string }
        const msg = e.detail ? `${e.error}: ${e.detail}` : e.error
        return { content: [{ type: 'text' as const, text: `Error: ${msg}` }], isError: true }
  ...
  defineTool(
    ...
    async (input, ctx) => {
      const { ok, data } = await ctx.callApi('POST', `/projects/${encodeURIComponent(input.projectId)}/run-lever-tick`, null)
      if (!ok) {
        const e = data as { error: string; detail?: string }
        const msg = e.detail ? `${e.error}: ${e.detail}` : e.error
        return { content: [{ type: 'text' as const, text: `Error: ${msg}` }], isError: true }
      ...
      const vitalsLine = r.vitals
        ? r.vitals.verdict === 'futile'
          ? `\nVitals: FUTILE — ${r.vitals.engaged} of ${r.vitals.sends} recent mature email sends drew a reply or inquiry-page engagement (P(dead) ${r.vitals.pDead.toFixed(3)}). Surface this in the cycle report: the user should check deliverability (DMARC, spam placement) and targeting before scaling sends.`
          : `\nVitals: ${r.vitals.verdict} (${r.vitals.engaged}/${r.vitals.sends} recent mature email sends drew a reply or inquiry-page engagement).`
  ...
  defineTool(
    ...
    async (input, ctx) => {
      ...
      const { ok, data } = await ctx.callApi('GET', `/projects/${encodeURIComponent(input.projectId)}/lever-state${qs}`, null)
      if (!ok) {
        const e = data as { error: string; detail?: string }
        const msg = e.detail ? `${e.error}: ${e.detail}` : e.error
        return { content: [{ type: 'text' as const, text: `Error: ${msg}` }], isError: true }
  ...
  defineTool(
    ...
    async (input, ctx) => {
      ...
      const { ok, data } = await ctx.callApi('GET', `/projects/${encodeURIComponent(input.projectId)}/lever-decisions${qs}`, null)
      if (!ok) {
        const e = data as { error: string; detail?: string }
        const msg = e.detail ? `${e.error}: ${e.detail}` : e.error
        return { content: [{ type: 'text' as const, text: `Error: ${msg}` }], isError: true }
  ...
  defineTool(
    ...
    async (input, ctx) => {
      const { ok, data } = await ctx.callApi('POST', '/outreach/skip', input)
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    async (input, ctx) => {
      const { ok, data } = await ctx.callApi('POST', '/notifications', input)
      if (!ok) {
        const err = data as { error: string; detail?: string }
        const msg = err.detail ? `${err.error}: ${err.detail}` : err.error
        return { content: [{ type: 'text' as const, text: `Error: ${msg}` }], isError: true }
  ...
  defineTool(
    ...
    async ({ ids, projectId }, ctx) => {
      ...
      const { ok, data } = await ctx.callApi('POST', '/outreach/drafts/discard', body)
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    async ({ projectId, limit, offset }, ctx) => {
      const { ok, data } = await ctx.callApi(
        'GET',
  ...
  defineTool(
    ...
    async ({ projectId, prospectId, status }, ctx) => {
      const { ok, data } = await ctx.callApi(
        'PATCH',
  ...
  defineTool(
    ...
    async ({ projectId, prospectId, priority }, ctx) => {
      const { ok, data } = await ctx.callApi(
        'PATCH',
  ...
  defineTool(
    ...
    async ({ organizationId, patch }, ctx) => {
      ...
      const { ok, data } = await ctx.callApi('PATCH', `/organizations/${organizationId}`, patch)
      if (!ok) {
        const e = data as { error: string; detail?: string }
        const msg = e.detail ? `${e.error}: ${e.detail}` : e.error
        return { content: [{ type: 'text' as const, text: `Error: ${msg}` }], isError: true }
  ...
  defineTool(
    ...
    async ({ domain, name, websiteUrl, doNotContact }, ctx) => {
      const { ok, data } = await ctx.callApi('POST', '/organizations', { domain, name, websiteUrl, doNotContact })
      if (!ok) {
        const e = data as { error: string; detail?: string }
        const msg = e.detail ? `${e.error}: ${e.detail}` : e.error
        return { content: [{ type: 'text' as const, text: `Error: ${msg}` }], isError: true }
  ...
  defineTool(
    ...
    async ({ q, limit, offset }, ctx) => {
      ...
      const { ok, data } = await ctx.callApi('GET', `/organizations?${params.toString()}`, null)
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    async ({ organizationIds }, ctx) => {
      const { ok, data } = await ctx.callApi('POST', '/organizations/delete-batch', { organizationIds })
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    async ({ prospectIds }, ctx) => {
      const { ok, data } = await ctx.callApi('POST', '/prospects/delete-batch', { prospectIds })
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    async ({ prospectId, patch }, ctx) => {
      const { ok, data } = await ctx.callApi('PATCH', `/prospects/${prospectId}`, patch)
      if (!ok) {
        const e = data as { error: string; detail?: string }
        const msg = e.detail ? `${e.error}: ${e.detail}` : e.error
        return { content: [{ type: 'text' as const, text: `Error: ${msg}` }], isError: true }
  ...
  defineTool(
    ...
    async ({ prospectId, doNotContact }, ctx) => {
      const { ok, data } = await ctx.callApi(
        'PATCH',
  ...
  defineTool(
    ...
    async ({ projectId, limit }, ctx) => {
      const { ok, data } = await ctx.callApi('GET', `/projects/${encodeURIComponent(projectId)}/outreach/recent?limit=${limit}`, null)
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    async (input, ctx) => {
      const { ok, data } = await ctx.callApi('POST', '/responses', input)
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    async ({ projectId, windowDays, scope }, ctx) => {
      const params = new URLSearchParams()
      if (windowDays != null) params.set('windowDays', String(windowDays))
      if (scope != null) params.set('scope', scope)
      const qs = params.toString() ? `?${params.toString()}` : ''
      const { ok, data } = await ctx.callApi('GET', `/projects/${encodeURIComponent(projectId)}/rejection-feedback/summary${qs}`, null)
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    async ({ projectId, status }, ctx) => {
      ...
      const { ok, data } = await ctx.callApi('GET', `/projects/${encodeURIComponent(projectId)}/suggestions${qs}`, null)
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    async ({ projectId, slug }, ctx) => {
      const { ok, status, data } = await ctx.callApi('GET', `/projects/${encodeURIComponent(projectId)}/documents/${slug}`, null)
      if (!ok) {
        if (status === 404) {
  ...
  defineTool(
    ...
    async ({ projectId }, ctx) => {
      const { ok, data } = await ctx.callApi('GET', `/projects/${encodeURIComponent(projectId)}/documents`, null)
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    async ({ slug }, ctx) => {
      const { ok, status, data } = await ctx.callApi('GET', `/master-documents/${slug}`, null)
      if (!ok) {
        if (status === 404) {
  ...
  defineTool(
    ...
    {},
    async (_args, ctx) => {
      const { ok, data } = await ctx.callApi('GET', '/master-documents', null)
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    async ({ excludeProjectId, q, industry, limit }, ctx) => {
      const params = new URLSearchParams()
      ...
      const { ok, data } = await ctx.callApi('GET', `/tenant/prospects?${params.toString()}`, null)
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    async ({ projectId, status, priority, q, scope, limit, offset }, ctx) => {
      ...
      const { ok, data } = await ctx.callApi('GET', `/projects/${encodeURIComponent(projectId)}/prospects?${params.toString()}`, null)
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  defineTool(
    ...
    async ({ projectId, ...patch }, ctx) => {
      const { ok, data } = await ctx.callApi('PUT', `/projects/${encodeURIComponent(projectId)}/settings`, patch)
      if (!ok) {
        const err = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${err.error}` }], isError: true }
      }
  ...
  const jobLine = (j: JobWire): string => {
    const progress = j.progress ? ` — ${j.progress.step} ${j.progress.done}${j.progress.total !== null ? `/${j.progress.total}` : ''}` : ''
  ...
  defineTool(
    ...
    'Start a server-run job for a project and answer with its id and status line. Kinds: daily_cycle (evaluate → lever tick → outbound drafting/sending → prospect discovery when the list runs low → journal), discover (find and register N new prospects, following the tick\'s strategy plan unless strategySlug pins one), enrich (read the candidates\' sites for contacts and register them), draft (compose outreach for the next N reachable prospects or the given prospectIds; the project\'s outbound mode decides draft vs send), send (send the given pending drafts), evaluate, journal. Runs in the background — poll get_job. The daily cycle refuses a second start while one is still running for the project.',
    {
      ...
      threadId: z.string().min(1).max(64).optional().describe('Chat thread to notify on completion; the hosted chat sets this.'),
    },
    async ({ projectId, params, threadId }, ctx) => {
      const { ok, data } = await ctx.callApi('POST', '/jobs', { projectId, params, threadId })
      if (!ok) {
        const e = data as { error: string; detail?: string }
        return { content: [{ type: 'text' as const, text: `Error: ${e.detail ? `${e.error}: ${e.detail}` : e.error}` }], isError: true }
      }
  ...
  defineTool(
    'list_jobs',
    'Recent jobs, newest first, one status line each; filter by project, status, or kind.',
    {
      projectId: z.string().min(1).optional().describe('Project name or ID'),
      status: z.enum(JOB_STATUSES).optional(),
      kind: z.enum(JOB_KINDS).optional(),
      limit: z.number().int().min(1).max(100).default(20),
    },
    async ({ projectId, status, kind, limit }, ctx) => {
      const qs = new URLSearchParams({ limit: String(limit) })
      if (projectId) qs.set('projectId', projectId)
      if (status) qs.set('status', status)
      if (kind) qs.set('kind', kind)
      const { ok, data } = await ctx.callApi('GET', `/jobs?${qs}`, null)
      if (!ok) {
        const e = data as { error: string }
        return { content: [{ type: 'text' as const, text: `Error: ${e.error}` }], isError: true }
      }
  ...
  defineTool(
    'cancel_job',
    'Stop a queued or running job. Work already done stays (registered prospects, stored drafts); answers with the job\'s status line.',
    { id: z.string().min(1).describe('Job id') },
    async ({ id }, ctx) => {
      const { ok, data } = await ctx.callApi('POST', `/jobs/${encodeURIComponent(id)}/cancel`, {})
      if (!ok) {
        const e = data as { error: string; detail?: string }
        return { content: [{ type: 'text' as const, text: `Error: ${e.detail ? `${e.error}: ${e.detail}` : e.error}` }], isError: true }
      }
      return { content: [{ type: 'text' as const, text: `Cancelled: ${jobLine(data as JobWire)}` }] }
    },
  )
  ...
  defineTool(
    ...
    async ({ projectId, draft }, ctx) => {
      ...
      if (!ok) {
        const e = data as { error: string; detail?: unknown }
        const detail = typeof e.detail === 'string' ? e.detail : e.detail ? JSON.stringify(e.detail) : ''
        return { content: [{ type: 'text' as const, text: `Error: ${e.error}${detail ? `: ${detail}` : ''}` }], isError: true }
      }
