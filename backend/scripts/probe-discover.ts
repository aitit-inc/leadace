...
async function measure(plan: (typeof strategies)[number]): Promise<void> {
  ...
  for (let i = 0; i < candidates.length; i += 4) {
    enriched.push(...(await Promise.all(candidates.slice(i, i + 4).map((c) => enrichCandidate(env, c, { procedure, offer, salesStrategy, approaches: [plan.approach], channels: ['email'] })))))
  }
