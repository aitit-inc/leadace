...
<div class="space-y-1.5 text-xs">
  ...
  {#if p.email}<p class="break-all"><span class="text-text-muted">Email:</span> <span class="font-mono">{p.email}</span>{#if p.emailNoSolicitation}<span class="ml-1 text-danger">(no-solicitation notice — never written to)</span>{/if}</p>{/if}
  {#if p.contactFormUrl}<p class="break-all"><span class="text-text-muted">Form:</span> {#if safeForm}<a href={safeForm} target="_blank" rel="noopener noreferrer" class="text-accent hover:underline">{p.contactFormUrl}</a>{:else}<span class="font-mono text-text-muted">{p.contactFormUrl}</span>{/if}{#if p.formNoSolicitation}<span class="ml-1 text-danger">(no-solicitation notice — never submitted)</span>{/if}</p>{/if}
