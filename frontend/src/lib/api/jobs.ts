...
import type { Job, JobDetail } from '$lib/types/jobs';

export function getJob(id: string, fetchFn: RequestFetch = fetch, token?: string): Promise<JobDetail> {
  return request<JobDetail>(fetchFn, { method: 'GET', path: `/jobs/${encodeURIComponent(id)}`, auth: 'required', token });
}
...
export function listProjectJobs(projectId: string, fetchFn: RequestFetch = fetch, token?: string): Promise<Job[]> {
  const sp = new URLSearchParams({ projectId, limit: '30' });
  return request<{ jobs: Job[] }>(fetchFn, { method: 'GET', path: `/jobs?${sp}`, auth: 'required', token }).then((r) => r.jobs);
}
