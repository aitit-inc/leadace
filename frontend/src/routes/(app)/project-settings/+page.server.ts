...
export const load: PageServerLoad = async ({ fetch, parent, depends, locals }) => {
  ...
  if (!activeProjectId) {
    return { projectId: null, projectSettings: null, sendingIdentities: [], sendingIdentitiesError: false, schedules: [] };
  }
  ...
  const [projectSettings, identities, schedules] = await Promise.all([
