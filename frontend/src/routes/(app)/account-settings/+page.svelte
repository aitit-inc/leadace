...
<div class="mx-auto max-w-4xl space-y-6">
  <h2 class="font-display text-2xl font-semibold tracking-tight text-text">Account</h2>

  <section class="card p-5">
    <h3 class="font-display text-lg font-semibold text-text">Google account</h3>
    <div class="mt-4 flex items-center gap-4">
      ...
      {:else}
        <div
          class="flex h-10 w-10 items-center justify-center rounded-full bg-surface-2 text-sm font-semibold text-text-secondary"
        >
          {profile.name.charAt(0).toUpperCase()}
        </div>
      {/if}
      <div class="min-w-0">
        <p class="truncate text-sm font-semibold text-text">{profile.name}</p>
        <p class="truncate text-sm text-text-secondary">{profile.email}</p>
      </div>
    </div>
  </section>
  ...
  <section class="card p-5">
    <div class="flex items-center gap-1.5">
      <h3 class="font-display text-lg font-semibold text-text">Connected MCP clients</h3>
    ...
    <div class="-mx-5 mt-4 border-t border-border">
      {#if data.mcpSessions.error}
        <p class="px-5 py-3 text-sm text-danger">{data.mcpSessions.error}</p>
      {:else if data.mcpSessions.sessions.length === 0}
        <p class="px-5 py-3 text-sm text-text-muted">
        ...
        <ul class="divide-y divide-border">
          {#each data.mcpSessions.sessions as session (session.familyId)}
            ...
            <li class="flex items-center justify-between gap-4 px-5 py-3">
              <div class="min-w-0">
                <p class="truncate text-sm font-semibold text-text">{label}</p>
    ...
    {#if mcpSessionMessage}
      <p class="mt-2 text-sm {mcpSessionMessage.startsWith('Error') ? 'text-danger' : 'text-text-muted'}">
        {mcpSessionMessage}
      </p>
    {/if}
  </section>
