...
{#snippet stat(label: string, value: string, hint: string | null, inbound: boolean = false)}
  <div class="card p-4">
    <div class="text-xs font-semibold text-text-muted">{label}</div>
    <div class="mt-1 font-display text-3xl font-semibold tabular-nums {inbound ? 'text-inbound' : 'text-text'}">{value}</div>
    {#if hint}
      <div class="mt-1 text-xs text-text-muted">{hint}</div>
    {/if}
  </div>
{/snippet}
...
<div class="min-h-screen bg-page">
  <div class="mx-auto max-w-3xl px-6 py-8">
    <header class="flex items-center justify-between gap-4">
      <a href="https://leadace.ai" class="inline-flex items-center gap-2 text-text">
    ...
    <section class="mt-10">
      <p class="text-sm font-semibold text-accent-strong">Live</p>
      <h1 class="mt-2 font-display text-3xl font-semibold tracking-tight text-text">{AGENT_NAME} is selling LeadAce.</h1>
    ...
    {#if board}
      <section class="mt-8">
        ...
        <h2 class="mt-6 font-display text-xl font-semibold text-text">Where the emails went</h2>
        <div class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          {@render stat('Positive', String(board.replies.positive), 'meetings and clear interest', true)}
        </div>

        {#if board.signups}
          <h2 class="mt-6 font-display text-xl font-semibold text-text">And what that turned into</h2>
          <div class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {@render stat('Signups today', String(board.signups.today), null, true)}
            {@render stat('Signups total', board.signups.total.toLocaleString(), 'LeadAce accounts', true)}
          </div>
        {/if}
      </section>
      ...
      <section class="card mt-8 p-5">
        <div class="flex items-baseline justify-between">
          <h2 class="font-display text-xl font-semibold text-text">Last 7 days</h2>
          <span class="flex items-center gap-3 text-xs text-text-muted">
            <span class="inline-flex items-center gap-1.5"><span class="h-2 w-2 rounded-full bg-text-muted/40"></span>sent</span>
            <span class="inline-flex items-center gap-1.5"><span class="h-2 w-2 rounded-full bg-inbound"></span>replies</span>
          </span>
        </div>
        <div class="mt-4 flex h-28 gap-1.5">
          {#each board.daily as day (day.date)}
            ...
            >
              <div
                class="w-1/2 rounded-sm bg-text-muted/40"
                style="height: {Math.round((day.sent / trendMax) * 100)}%"
              ></div>
              <div
                class="w-1/2 rounded-sm bg-inbound"
        ...
        <div class="mt-2 flex gap-1.5">
          {#each board.daily as day (day.date)}
            <div class="flex-1 text-center text-xs tabular-nums text-text-muted">
              {shortDate(day.date)}
            </div>
          {/each}
        </div>
      ...
      <section class="mt-8">
        <div class="flex items-baseline justify-between">
          <h2 class="font-display text-xl font-semibold text-text">{AGENT_NAME}'s journal</h2>
          {#if board.journal}
        ...
        {#if board.journal}
          <div
            class="journal card mt-3 p-5 text-base leading-relaxed text-text"
          >
        ...
        {:else}
          <p class="card mt-3 p-5 text-sm text-text-muted">
    ...
    {:else}
      <section class="card mt-8 p-6 text-sm text-text-muted">
    ...
    <section class="card mt-12 p-6">
      <h2 class="font-display text-xl font-semibold text-text">Want {AGENT_NAME} selling your product?</h2>
    ...
    <footer class="mt-10 text-xs leading-relaxed text-text-muted">
